...
Убрать программу с сервера
  На сервере:
    sudo defendra uninstall
  Enter — оставить. Нужно явно написать да.
  --yes этот шаг не делает.
  Снимет программу и её настройки. Пользователя admin не трогает.
  Фильтр и защиту от подбора пароля пакетами не удаляет.
  Если есть снимок — вернёт вход и фильтр как до последней настройки.
